async function createClubList(data) {
    const contentPlace = document.querySelector('.content')
    const clubs = data
    for (let i = 0; i < clubs.length; i++) {
        const newElement = `
                           <section class="club" data-type="club" id="${clubs[i].id}">
                            <div class="club__info">
                                <div class="club__image">
                                    <img src="../assets/img/group.svg" alt="">
                                </div>
                                <div class="club__content">
                                    <h2><a data-target="club-page" id="${clubs[i].id}" data-crumb="${clubs[i].name}">${clubs[i].name}</a></h2>
                                    <h3>${clubs[i].description}</h3>
                                    <div class="club__more">
                                        <p>Общая сумма: <span data-sum>${clubs[i].costInfo.certificateCost}</span> &#8381;</p>
                                        <p>Стоимость участия: <span data-cost>${clubs[i].costInfo.entryCost}</span> &#8381;</p>
                                    </div>
                                </div>
                            </div>
                            <div class="club__link">
                                ...
                                <a data-target="club-page" id="${clubs[i].id}" data-crumb="${clubs[i].name}">Подробнее</a>
                            </div>
                        </section>
            `
        contentPlace.insertAdjacentHTML("beforeend", newElement)
        const newDOMElement = findSection('club', `${clubs[i].id}`)
